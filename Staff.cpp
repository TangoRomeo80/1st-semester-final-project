...
void Staff::AddPc() { // Add new Pcs
    system("cls");
    Pc newPc;
    string money, number, category, price, name, model, author, discount, disc_num, information;
    cout << "Please input the model number of the Pc" << endl;
    cin >> model; // Input model number of this new Pc
    ...
    cin >> name; // Input Pcname
    cout << "Please input the usage of the Pc" << endl;
    cin >> author; // Input usage
    ...
    while (1) {
        cin >> money; // Input the cost of those Pc
        if (IsAllDigit(money)) break;
        cout << "Pleass input a digital number :)   Try again." << endl;
    }
    newPc.SetCName(name);
    newPc.SetModel(model);
    newPc.SetUsage(author);
    newPc.SetNumber(stoi(number));
    newPc.SetCategory(stoi(category));
    ...
    cout << "Add Pc succsessfully!~ " << endl;
    Sleep(1000);
...
void Staff::DelPc() { // Delete some Pcs
    ...
    cin >> model; // Input model number of the Pc that you want to delete
    for (size_t i = 0; i < lib.PcArray.size(); i++)
        if (lib.PcArray[i].GetModel() == model) {
            cout << "There are " << lib.PcArray[i].GetNumber() << " copies of this Pc, how many do you want to delete?  " << endl;
            while (1) {
                cin >> temp; // Input how many Pcs you want to delete
            ...
            if (stoi(temp) > lib.PcArray[i].GetNumber()) { // Pc number is not enough to delete
                ...
                cout << "Delete (" << stoi(temp) << ") of this Pc succsessfully!~ " << endl;
                Sleep(1000);
                cout << "Automatic quit after 3 seconds." << endl;
                Sleep(2000);
...
void Staff::SetPc() { // Change Pcs' information
    system("cls");
    int choice;
    string number, category, price, name, model, author, discount, disc_num, information;
    cout << "Please input the model number of the Pc that you want to change the information." << endl;
    cin >> model; // Input model number of the Pc that you want to change information
    for (size_t i = 0; i < lib.PcArray.size(); i++)
        if (lib.PcArray[i].GetModel() == model ) {
            ...
            if (choice == 1) {
                ...
                cin >> model; // Input new model number
            ...
            if (choice == 1) {
                ...
                cin >> author; // Input new usage
                lib.PcArray[i].SetUsage(author);
            }
            ...
            if (choice == 1) {
                ...
                lib.PcArray[i].SetCategory(stoi(category));
            }
            ...
            cout << "Change Pc information succsessfully!~ " << endl;
            Sleep(1000);
            cout << "Automatic quit after 3 seconds." << endl;
            Sleep(2000);
