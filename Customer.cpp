...
void Customer::BuyPc() { // Customer buy Pcs
    ...
    getline(cin,model); // Input the model number
    for (size_t i = 0; i < lib.PcArray.size(); i++)
        if (lib.PcArray[i].GetModel() == model) { // If find that model number
            cout << "There are " << lib.PcArray[i].GetNumber() << " numbers of this Pc, how many do you want to buy?  " << endl;
            while (1) {
            ...
            if (stoi(temp) > lib.PcArray[i].GetNumber()) { // If Pc is not enough for customer to buy
                ...
                cout << "Buying " << stoi(temp) << " numbers of that Pc, successful!~ " << endl;
                Sleep(1000);
    ...
    cout << "Could not find this Pc! " << endl; // Pc not found in Pc.txt
    Sleep(1000);
}
...
void Customer::Refund() { // Customer ask for refund
    ...
    for (size_t i = 0; i < lib.PcArray.size(); i++)
        if (lib.PcArray[i].GetModel() == model) { // If find that model number
            cout << " How many copies of that Pc do you want to ask for refund?  " << endl;
            while (1) {
            ...
            cout << "Refunded " << stoi(temp) << " numbers of that Pcs, successfully!~ " << endl;
            Sleep(1000);
    ...
    cout << "This is not in our store! " << endl; // Pc not found in Pc.txt
    Sleep(1000);
}
