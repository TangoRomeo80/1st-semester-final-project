...
void User::PrintPcArray() { // Output Pc information to Pc.txt
    ...
    for (size_t i = 0; i < lib.PcArray.size(); i++ ) // Output each Pc's information
        fout << lib.PcArray[i].GetName() << " " << lib.PcArray[i].GetIsbn() << " " << lib.PcArray[i].GetAuthor()
...
void User::ListPcAll() { // List all Pc
    ...
    for (size_t i = 0; i < lib.PcArray.size(); i++) { // Output each Pc information
        cout << "|" << setw(18) << lib.PcArray[i].GetIsbn() << "|" <<  setw(29) << lib.PcArray[i].GetName() <<  "|"
...
void User::ListPcByN() { // List Pc by name
    system("cls");
    string temp, name;
    cout << "Please input the name that you want to find: " << endl;
    cin >> name;
    system("cls");
    cout << "This is the list of all the Pc that name contains string \"" << name << " \" :" << endl;
    cout << "-------------------------------------------------------------------------" << endl;
    cout << "|       ISBN       |             Name            | Number |    Price    |" << endl;
    cout << "-------------------------------------------------------------------------" << endl;
    for (size_t i = 0; i < lib.PcArray.size(); i++) // Output each Pc information match this name
        if (lib.PcArray[i].GetName().find(name) != string::npos) {
            cout << "|" << setw(18) << lib.PcArray[i].GetIsbn() << "|" <<  setw(29) << lib.PcArray[i].GetName() <<  "|"
                 <<  setw(8) << lib.PcArray[i].GetNumber() << "|"<<  setw(13) << setiosflags(ios::fixed) << setprecision(2) <<lib.PcArray[i].GetPrice()  << "|" << endl;
        }
    cout << "-------------------------------------------------------------------------" << endl;
...
void User::PcInfor() { // See more information of one Pc
    system("cls");
    string temp, isbn;
    cout << "Please input the ISBN number of the Pc that you want to find: " << endl;
    cin >> isbn;
    for (size_t i = 0; i < lib.PcArray.size(); i++)
        if (lib.PcArray[i].GetIsbn() == isbn ) { // If exist, list its detail information
            system("cls");
            cout << "Detail information of the Pc with ISBN number " << isbn << " : " << endl;
            cout << "Name            : " << lib.PcArray[i].GetName() << endl;
            cout << "Author            : " << lib.PcArray[i].GetAuthor() << endl;
